<!--
Author: Jeff
Date: 2026-05-27
Description: Index for Spectre Seq project-local agents
Notes: Keep this list synchronized when adding, removing, or renaming local agents
-->

# Spectre Seq Agents Index

Use the narrowest agent that owns the current slice.

## Agents

- `spectre-seq-orchestrator`: phase sequencing, task decomposition, final integration review.
- `spectre-core-graph-agent`: `spectre-core` and `spectre-graph` implementation/review.
- `spectre-audio-dsp-agent`: audio backend, DSP, synth/fx/modular implementation/review.
- `spectre-plugin-host-agent`: CLAP/LV2 plugin host implementation/review.
- `spectre-ui-project-agent`: timeline, automation, persistence, UI implementation/review.
- `spectre-reviewer`: independent gatekeeper for plan fidelity and quality.
