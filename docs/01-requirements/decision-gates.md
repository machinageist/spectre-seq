...
| 8 | UI stack | Custom-drawn musician-facing DAW surfaces, reusing platform/rendering infrastructure for windowing, GPU, text, and accessibility rather than building those foundations from scratch. Jeff explicitly selected this boundary on 2026-09-07. egui remains the existing R4 shell, not a ratified final UI stack; toolkit selection and migration gates need a comparative technical evaluation. Keyboard-complete operation and the before-beta accessibility gate remain binding. | Low once beta ships | **Accepted direction**, clarified by Jeff 2026-09-07; supersedes the requirement that custom rendering be justified only by measured egui limits. Concrete stack and migration plan remain open |
