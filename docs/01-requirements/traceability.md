...
| RT-001..003 | workspace policy; active R3 scope as of 2026-08-09. The seam's own render path has an allocation gate; no callback-reachable guard covers a live driver yet | `pump_alloc.rs` steady-state allocation gate on the null render path | RT-001..002 accepted, RT-003 proposed; none satisfied |

Full-gate result (2026-08-09, repository root, rustc/cargo 1.97.1, after the Geist → Spectre rename and R3 slice 2): formatting clean; strict Clippy clean; 170/170 tests pass, of which 15 are the new backend seam; `./spectre --smoke-test` identifies Pulse selection and the offline self-test passes. The interaction evidence includes 27/27 app-model tests and 1/1 process smoke test; the offline harness remains 21/21. R2 silence/impulse/allocation/hash gates remain green on the unchanged compiled-plan path.
