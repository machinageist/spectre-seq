...
| RT-001..003 | workspace policy only; active R3 scope as of 2026-08-09 | — | RT-001..002 accepted, RT-003 proposed; none implemented |

Full-gate result (2026-08-09, repository root, rustc/cargo 1.97.1, re-run after the Geist → Spectre rename): formatting clean; strict Clippy clean; 155/155 tests pass; `./spectre --smoke-test` identifies Pulse selection and the offline self-test passes. The interaction evidence includes 27/27 app-model tests and 1/1 process smoke test; the offline harness remains 21/21. R2 silence/impulse/allocation/hash gates remain green on the unchanged compiled-plan path.
