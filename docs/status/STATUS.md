...
- **Known gaps:** no live driver has ever been opened, so backend qualification and the hardware lifecycle drill are the two outstanding R3 exit rows; `./spectre` does not use the audio crate; live parameter application is deferred to R4 by decision 22
