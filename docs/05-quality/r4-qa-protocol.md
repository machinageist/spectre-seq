...
| 8 | **Save over an existing file** | Save onto an existing project file. The previous file is either fully replaced or fully intact — never half-written. Also confirm the directory holds no leftover **temporary** — a `spectre-tmp` sibling. **Amended 2026-08-29:** this row read "exactly one file afterward" until an autosave sidecar existed. `journal.rs` writes `<project>.autosave` beside the project by design, so a second file may be present and is not a defect; what must not be present is a temporary. If the shell has not yet called `write_autosave`, no sidecar appears and that is also correct |
