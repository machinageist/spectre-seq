...
- ~~A MIDI clip plays through a track into master~~ — **partially closed 2026-08-28.** The clip model, the baked schedule, and the merge rule are evidenced; `crates/spectre-offline/tests/e2e_alpha.rs` renders three tracks' clips into master offline. The live path composes too: `the_live_bridge_plays_every_instrument_track` drives all three tracks' clips through `RenderBridge` and matches the offline hash exactly, at zero callback allocations. **Open in the product:** `./spectre` attaches no clip player to its audition path, so the shell still plays a held audition note rather than the project's clips. Whichever slice attaches one must replace the audition note rather than merge with it.
