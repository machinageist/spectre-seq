...
Reordering requires documented dependency reasoning. **R5 is active as of 2026-08-29.** R4 did not exit: eight of its ten rows are closed and it carries a named `R4-exit-pending-operator-QA` state until an operator works `../05-quality/r4-qa-protocol.md`. R5 opened anyway because no R5 obligation depends on that pass — the reasoning the first sentence of this paragraph requires is written out in `current-milestone.md` §"Why R5 opens while one R4 row is open", and follows D-R4's disposition (c). R0/R1 exited 2026-07-17, R2 exited 2026-08-09 on its four render gates, and R3 exited 2026-08-09 with all ten rows closed — two of them on macOS hardware only, per decision 23, with the Linux device drill carried to R4 as debt.
